...
class RedisBroadcaster implements Broadcaster
{
    ...
    {
        //Log::info("redis channel ".print_r($subscriber,true));
        $pub['event'] = $subscriber->operationName;
        $pub['data'] = $data;
        //Log::info("redis channel - ".json_encode($pub));

        //Redis::publish($subscriber->channel, json_encode($pub));
        broadcast(new Message($pub));
